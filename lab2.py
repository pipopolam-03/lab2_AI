...
def A(start, target):
    ...
    while True:
        ...
        if 'd' in moves:
            ...
            # условие - проверяем матрицу родителя, чтобы не ходить кругами и проверяем, не попадем 
            # ли в пройденное состояние, если пойдет по этому пути
            if str(up_state.matrix) not in passed  and down_node.state.matrix != current_node.parent.state.matrix:
        ...
        if 'r' in moves:
            ...
            # условие - проверяем матрицу родителя, чтобы не ходить кругами и проверяем, не попадем 
            # ли в пройденное состояние, если пойдет по этому пути
            if str(up_state.matrix) not in passed and right_node.state.matrix != current_node.parent.state.matrix:
        ...
        if 'l' in moves:
            ...
            # условие - проверяем матрицу родителя, чтобы не ходить кругами и проверяем, не попадем 
            # ли в пройденное состояние, если пойдет по этому пути
            if str(up_state.matrix) not in passed and left_node.state.matrix != current_node.parent.state.matrix:
